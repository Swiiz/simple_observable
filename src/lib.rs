...
pub type Observer<'a, T> = <T as Observable>::Observer<'a>;
pub type AsIterObserver<'a, T> = <ChangeIter<T> as Observable>::Observer<'a>;
...
pub trait ObservableAsIter: AsIter {
    fn pull_changes<'a>(
        &'a self,
